...
def get_weather(city: str, state: Optional[str], country: str, units: str) -> Optional[dict]:
    ...
    data: Optional[Dict[Any,Any]] = __cache.get(key)
    if not data:
        return None
...
def set_weather(city: str, state: Optional[str], country: str, units: str, value: Optional[Dict[Any,Any]])->None:
    key = __create_key(city, state, country, units)
    data = {
